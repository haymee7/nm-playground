...
public class BlogServiceImpl implements BlogService {
  ...
  public ResponseEntity<ResponseDto<BlogResDto>> retrieveOne(int pid) {
    ...
    BlogDto blogDto = Optional.ofNullable(blogDao.retrieveOne(pid)).orElseThrow(() -> new ApiException(COMM_RETRIVE_NOT_EXIST));
  ...
  @Override
  public ResponseEntity<ResponseDto<BlogListResDto>> retrieveByCnt(int sp, int cnt) {
    checkRetrieveListParams(sp,cnt);
    BlogListResDto blogListResDto = new BlogListResDto();
    blogListResDto.setTotalCnt(blogDao.totalCnt());
    // BlogResDto만 모아서 새로운 리스트를 만들어 낼 수 있다.
    blogListResDto.setList(blogDao.retrieveByCnt(sp, cnt).stream().map(BlogResDto::new).collect(Collectors.toList()));

    ResponseDto<BlogListResDto> responseDto = new ResponseDto<>(true);
    responseDto.setData(blogListResDto);
    return ResponseEntity.ok(responseDto);
  }
